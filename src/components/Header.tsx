...
const Header = ({
    ...
    const displayPanel = activePanel === 'commentary' ? 'commentary' : 'reflections';
    const nextPanel = activePanel ? (displayPanel === 'commentary' ? 'reflections' : 'commentary') : 'reflections';

    const panelMeta =
        displayPanel === 'commentary'
            ? {
                  label: 'Commentary',
                  title: '코멘터리 전환',
            ...
            <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-3 sm:px-5">
                <div className="flex min-w-0 flex-1 items-center gap-1 text-text-primary dark:text-dark-text-primary sm:gap-2">
                    ...
                    <Link to={targetUrl} className="group flex min-w-0 items-center gap-1 truncate sm:gap-2.5">
                        <span className="shrink-0 font-serif text-[24px] leading-none text-gold-primary opacity-90 transition-transform duration-700 group-hover:rotate-90 sm:text-[28px]">
                            ॐ
                        </span>
                ...
                <div className="ml-2 flex shrink-0 items-center gap-1.5 sm:ml-3 sm:gap-3">
                    ...
                    {showSidebarToggle && (
                        ...
                        >
                            ...
                            <span className="hidden leading-none min-[420px]:inline">{panelMeta.label}</span>
                        </button>
                    )}
