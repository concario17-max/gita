...
const CommentarySidebar = () => {
    ...
    return (
        <>
            ...
            <aside
                className={`fixed inset-y-0 right-0 z-50 flex h-[100dvh] flex-col border-l border-gold-primary/20 bg-white/40 font-inter backdrop-blur-md transition-all duration-300 dark:border-dark-border/50 dark:bg-dark-surface/40 sm:w-[400px] lg:sticky lg:top-16 lg:h-[calc(100vh-64px)]
                ${isOpen ? 'w-[90vw] translate-x-0 overflow-hidden shadow-2xl lg:shadow-none' : 'w-[90vw] translate-x-full lg:translate-x-0'}
                ${isDesktopOpen ? `${desktopWidthClass} lg:opacity-100` : 'lg:w-0 lg:translate-x-10 lg:border-none lg:opacity-0 overflow-hidden px-0'}`}
            >
                ...
                <div className="relative flex h-full min-h-0 flex-col p-6">
                    ...
                    <div className="mb-4 flex flex-1 min-h-0 flex-col space-y-2">
                        ...
                        <div className="custom-scrollbar flex-1 overflow-y-auto rounded-2xl border border-gold-primary/20 bg-white/70 p-5 font-sans text-base leading-loose text-text-primary shadow-inner backdrop-blur-sm transition-all dark:border-dark-border/60 dark:bg-dark-bg/60 dark:text-dark-text-primary">
                            <p className="mt-10 text-center opacity-50">이곳에 코멘터리가 표시될 예정입니다.</p>
                        </div>
                    </div>
                </div>
            </aside>
