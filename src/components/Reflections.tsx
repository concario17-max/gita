...
const Reflections = () => {
    ...
    const handleExportCurrent = () => {
        downloadTextFile(`Yoga_Sutras_Reflection_Chapter_${chapterNum}_Sutra_${verseNum}.txt`, note);
    ...
    const handleExportAll = () => {
        ...
        noteKeys.sort((a, b) => {
            ...
            if (parseInt(chA, 10) !== parseInt(chB, 10)) {
                return parseInt(chA, 10) - parseInt(chB, 10);
            }
        ...
        noteKeys.forEach((key) => {
            ...
            if (content && content.trim()) {
                allNotesText += `--- Chapter ${chapter}, Sutra ${verse} ---\n${content}\n\n`;
            }
        });
        ...
        downloadTextFile('Yoga_Sutras_All_Reflections.txt', allNotesText);
        setShowExportMenu(false);
    };
    ...
    return (
        <>
            ...
            <aside
                ...
                ${isDesktopReflectionsOpen ? 'lg:w-[400px] lg:opacity-100' : 'overflow-hidden px-0 lg:w-0 lg:translate-x-10 lg:border-none lg:opacity-0'}`}
            >
                ...
                <div className="relative flex h-full min-h-0 flex-col p-6">
                    <div className="mb-6 flex shrink-0 items-center gap-2 border-b border-gold-border/30 pb-4">
                        ...
                        <h2 className="text-sm font-bold tracking-wide text-[#1C2B36] dark:text-dark-text-primary">수행 노트 (Reflections)</h2>
                    </div>

                    <div className="mb-4 flex min-h-0 flex-1 flex-col space-y-2">
                        <div className="text-xs font-bold tracking-wider text-[#8FA0AD]">{chapterNum}.{verseNum}</div>

                        <textarea
                            value={note}
                            onChange={(event) => setNote(event.target.value)}
                            placeholder="현재 구절을 읽으며 떠오른 해석, 질문, 통찰을 기록해 보세요."
